...
{
    private static void Conv3x3Stride2FourOutputsAvx512(ReadOnlySpan<float> input,
        ...
        for (int b = 0; b < batch; b++)
            for (int co = 0; co < outputChannels; co += 4)
            {
                ...
                for (int oy = 0; oy < outputHeight; oy++)
                {
                    ...
                    {
                        // Interior vectors: 16 gathered lanes (stride-2) need
                        // source indices first .. first+30 in bounds.
                        bool canVector = x + 16 <= outputWidth;
                        if (canVector)
                        {
                            for (int kx = 0; kx < 3; kx++)
                            {
                                int first = 2 * x - 1 + kx;
                                if (first < 0 || first + 30 >= inputWidth) { canVector = false; break; }
                            }
                        }
                        if (canVector)
                        {
                            Vector512<float> a0 = Vector512.Create(bias0);
                            Vector512<float> a1 = Vector512.Create(bias1);
                            Vector512<float> a2 = Vector512.Create(bias2);
                            Vector512<float> a3 = Vector512.Create(bias3);
                            for (int ci = 0; ci < inputChannels; ci++)
                            {
                                ...
                                for (int ky = 0; ky < 3; ky++)
                                {
                                    ...
                                    for (int kx = 0; kx < 3; kx++)
                                    {
                                        Vector512<float> value = LoadStride2512(source, sourceOffset + kx);
                            ...
                            Store512(output, output0 + row + x, a0);
                            Store512(output, output1 + row + x, a1);
                            Store512(output, output2 + row + x, a2);
                            Store512(output, output3 + row + x, a3);
                            x += 16;
                            continue;
                        }

                        for (int lane = 0; lane < Math.Min(16, outputWidth - x); lane++)
                        {
                            int ox = x + lane;
    ...
    private static unsafe void Conv3x3Stride2EightOutputsAvx512Unsafe(ReadOnlySpan<float> input,
        ReadOnlySpan<float> weights, ReadOnlySpan<float> bias, Span<float> output,
        int batch, int inputChannels, int inputHeight, int inputWidth,
        int outputHeight, int outputWidth, int outputChannels)
    {
                ...
                for (int co = 0; co < outputChannels; co += 8)
                {
                    ...
                    for (int oy = 0; oy < outputHeight; oy++)
                    {
                        ...
                        while (ox < outputWidth)
                        {
                            ...
                            bool fullWidth = ox != 0 && ox + 16 <= outputWidth && ((ox + 15) * 2 + 1 < inputWidth);
                            if (fullHeight && fullWidth)
                            {
                                ...
                                for (int ci = 0; ci < inputChannels; ci++)
                                {
                                    ...
                                    for (int ky = 0; ky < 3; ky++)
                                    {
                                        ...
                                        for (int kx = 0; kx < 3; kx++)
                                        {
                                            Vector512<float> value = LoadStride2512(srcRow + ix + kx);
                                            int wi = wb + ky * 3 + kx;
                                ...
                                Avx512F.Store(o0 + rowOffset + ox, a0); Avx512F.Store(o1 + rowOffset + ox, a1);
                                Avx512F.Store(o2 + rowOffset + ox, a2); Avx512F.Store(o3 + rowOffset + ox, a3);
                                Avx512F.Store(o4 + rowOffset + ox, a4); Avx512F.Store(o5 + rowOffset + ox, a5);
                                Avx512F.Store(o6 + rowOffset + ox, a6); Avx512F.Store(o7 + rowOffset + ox, a7);
                                ox += 16;
                            }
                            else
                            {
    ...
    private static void Conv3x3Stride2EightOutputsAvx512(ReadOnlySpan<float> input,
        ReadOnlySpan<float> weights, ReadOnlySpan<float> bias, Span<float> output,
        int batch, int inputChannels, int inputHeight, int inputWidth,
        int outputHeight, int outputWidth, int outputChannels)
    {
        ...
        for (int b = 0; b < batch; b++)
            for (int co = 0; co < outputChannels; co += 8)
            {
                ...
                Vector512<float> vb0 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 0]);
                Vector512<float> vb1 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 1]);
                Vector512<float> vb2 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 2]);
                Vector512<float> vb3 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 3]);
                Vector512<float> vb4 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 4]);
                Vector512<float> vb5 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 5]);
                Vector512<float> vb6 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 6]);
                Vector512<float> vb7 = Vector512.Create(bias.IsEmpty ? 0f : bias[co + 7]);
                for (int oy = 0; oy < outputHeight; oy++)
                {
                    int row = oy * outputWidth;
                    for (int x = 0; x < outputWidth;)
                    {
                        ...
                        if (canVector)
                        {
                            Vector512<float> a0 = vb0; Vector512<float> a1 = vb1; Vector512<float> a2 = vb2; Vector512<float> a3 = vb3;
                            Vector512<float> a4 = vb4; Vector512<float> a5 = vb5; Vector512<float> a6 = vb6; Vector512<float> a7 = vb7;
                            for (int ci = 0; ci < inputChannels; ci++)
                            {
                                ...
                                for (int ky = 0; ky < 3; ky++)
                                {
                                    ...
                                    {
                                        Vector512<float> value = LoadStride2512(source, sourceOffset + kx);
                                        a0 = AddMul512(a0, value, weights[w0 + ky * 3 + kx]); a1 = AddMul512(a1, value, weights[w1 + ky * 3 + kx]);
                                        a2 = AddMul512(a2, value, weights[w2 + ky * 3 + kx]); a3 = AddMul512(a3, value, weights[w3 + ky * 3 + kx]);
                                        a4 = AddMul512(a4, value, weights[w4 + ky * 3 + kx]); a5 = AddMul512(a5, value, weights[w5 + ky * 3 + kx]);
                                        a6 = AddMul512(a6, value, weights[w6 + ky * 3 + kx]); a7 = AddMul512(a7, value, weights[w7 + ky * 3 + kx]);
                                    }
                                }
                            }
                            Store512(output, o0 + row + x, a0); Store512(output, o1 + row + x, a1);
                            Store512(output, o2 + row + x, a2); Store512(output, o3 + row + x, a3);
                            Store512(output, o4 + row + x, a4); Store512(output, o5 + row + x, a5);
                            Store512(output, o6 + row + x, a6); Store512(output, o7 + row + x, a7);
                            x += 16; continue;
                        }
                        for (int lane = 0; lane < Math.Min(16, outputWidth - x); lane++)
                        {
    ...
    [MethodImpl(MethodImplCompat.AggressiveOptimization)]
    private static unsafe void Conv3x3Stride2SixteenOutputsPackedAvx512Unsafe(ReadOnlySpan<float> input,
        ...
        fixed (float* inputPtr = input, weightsPtr = packedWeights, biasPtr = bias, outputPtr = output)
        {
            for (int b = 0; b < batch; b++)
                for (int co = 0; co < outputChannels; co += 16)
                {
                    ...
                    Vector512<float> b0 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co]);
                    Vector512<float> b1 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 1]);
                    Vector512<float> b2 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 2]);
                    Vector512<float> b3 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 3]);
                    Vector512<float> b4 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 4]);
                    Vector512<float> b5 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 5]);
                    Vector512<float> b6 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 6]);
                    Vector512<float> b7 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 7]);
                    Vector512<float> b8 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 8]);
                    Vector512<float> b9 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 9]);
                    Vector512<float> b10 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 10]);
                    Vector512<float> b11 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 11]);
                    Vector512<float> b12 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 12]);
                    Vector512<float> b13 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 13]);
                    Vector512<float> b14 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 14]);
                    Vector512<float> b15 = Vector512.Create(biasPtr == null ? 0f : biasPtr[co + 15]);
                    float* batchInput = inputPtr + b * inputChannels * inputPlane;
                    for (int oy = 0; oy < outputHeight; oy++)
                    {
                        ...
                        while (ox < outputWidth)
                        {
                            bool vector = oy > 0 && oy * 2 + 1 < inputHeight && ox > 0 && ox + 16 <= outputWidth &&
                                (ox + 15) * 2 + 1 < inputWidth;
                            if (vector)
                            {
                                Vector512<float> a0 = b0, a1 = b1, a2 = b2, a3 = b3;
                                Vector512<float> a4 = b4, a5 = b5, a6 = b6, a7 = b7;
                                Vector512<float> a8 = b8, a9 = b9, a10 = b10, a11 = b11;
                                Vector512<float> a12 = b12, a13 = b13, a14 = b14, a15 = b15;
                                for (int ci = 0; ci < inputChannels; ci++)
                                {
                                    ...
                                    Vector512<float> value = LoadStride2512(row0);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8);
                                    value = LoadStride2512(row0 + 1);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 8);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 8);
                                    value = LoadStride2512(row0 + 2);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 16);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 16);
                                    value = LoadStride2512(row1);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 24);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 24);
                                    value = LoadStride2512(row1 + 1);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 32);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 32);
                                    value = LoadStride2512(row1 + 2);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 40);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 40);
                                    value = LoadStride2512(row2);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 48);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 48);
                                    value = LoadStride2512(row2 + 1);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 56);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 56);
                                    value = LoadStride2512(row2 + 2);
                                    AddEightPacked512(ref a0, ref a1, ref a2, ref a3, ref a4, ref a5, ref a6, ref a7, value, weights0 + 64);
                                    AddEightPacked512(ref a8, ref a9, ref a10, ref a11, ref a12, ref a13, ref a14, ref a15, value, weights8 + 64);
                                }
                                Avx512F.Store(o0 + row + ox, a0); Avx512F.Store(o1 + row + ox, a1);
                                Avx512F.Store(o2 + row + ox, a2); Avx512F.Store(o3 + row + ox, a3);
                                Avx512F.Store(o4 + row + ox, a4); Avx512F.Store(o5 + row + ox, a5);
                                Avx512F.Store(o6 + row + ox, a6); Avx512F.Store(o7 + row + ox, a7);
                                Avx512F.Store(o8 + row + ox, a8); Avx512F.Store(o9 + row + ox, a9);
                                Avx512F.Store(o10 + row + ox, a10); Avx512F.Store(o11 + row + ox, a11);
                                Avx512F.Store(o12 + row + ox, a12); Avx512F.Store(o13 + row + ox, a13);
                                Avx512F.Store(o14 + row + ox, a14); Avx512F.Store(o15 + row + ox, a15);
                                ox += 16;
                            }
                            else
                            {
    ...
    [MethodImpl(MethodImplCompat.AggressiveOptimization)]
    private static unsafe void Conv3x3Stride2EightOutputsPackedAvx512Unsafe(ReadOnlySpan<float> input,
        ReadOnlySpan<float> packedWeights, ReadOnlySpan<float> bias, Span<float> output, int batch,
        int inputChannels, int inputHeight, int inputWidth, int outputHeight, int outputWidth,
        int outputChannels)
    {
                        ...
                        while (ox < outputWidth)
                        {
                            ...
                            if (vector)
                            {
                                ...
                                for (int ci = 0; ci < inputChannels; ci++)
                                {
                                    ...
                                    float* row1 = row0 + inputWidth, row2 = row1 + inputWidth;
                                    Vector512<float> v0 = LoadStride2512(row0), v1 = LoadStride2512(row0 + 1), v2 = LoadStride2512(row0 + 2);
                                    float* weights = wc;
                                    ...
                                    weights += 8;
                                    v0 = LoadStride2512(row1); v1 = LoadStride2512(row1 + 1); v2 = LoadStride2512(row1 + 2);
                                    a0 = AddMul512(a0, v0, weights[0]); a1 = AddMul512(a1, v0, weights[1]);
                                    a2 = AddMul512(a2, v0, weights[2]); a3 = AddMul512(a3, v0, weights[3]);
                                    a4 = AddMul512(a4, v0, weights[4]); a5 = AddMul512(a5, v0, weights[5]);
                                    a6 = AddMul512(a6, v0, weights[6]); a7 = AddMul512(a7, v0, weights[7]);
                                    ...
                                    a6 = AddMul512(a6, v2, weights[6]); a7 = AddMul512(a7, v2, weights[7]);
                                    weights += 8;
                                    v0 = LoadStride2512(row2); v1 = LoadStride2512(row2 + 1); v2 = LoadStride2512(row2 + 2);
                                    a0 = AddMul512(a0, v0, weights[0]); a1 = AddMul512(a1, v0, weights[1]);
                                    a2 = AddMul512(a2, v0, weights[2]); a3 = AddMul512(a3, v0, weights[3]);
                                    a4 = AddMul512(a4, v0, weights[4]); a5 = AddMul512(a5, v0, weights[5]);
                                    a6 = AddMul512(a6, v0, weights[6]); a7 = AddMul512(a7, v0, weights[7]);
