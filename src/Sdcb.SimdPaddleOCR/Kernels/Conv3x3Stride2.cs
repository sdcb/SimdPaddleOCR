...
internal static partial class Conv3x3Stride2
{
        ...
        int outputChannels, int intraOpThreads = 1)
    {
        ...
        bool canShard = intraOpThreads > 1 && batch == 1 && blocks >= 2;

#if !NETSTANDARD2_0
        if (Avx512F.IsSupported)
        {
            if (canShard)
                TryPackedShard(input, packedWeights, bias, output, inputChannels, inputHeight,
                    inputWidth, outputHeight, outputWidth, outputChannels, intraOpThreads);
            else
    ...
    private static unsafe void TryPackedShard(ReadOnlySpan<float> input,
        ReadOnlySpan<float> packedWeights, ReadOnlySpan<float> bias, Span<float> output,
        int inputChannels, int inputHeight, int inputWidth, int outputHeight, int outputWidth,
        int outputChannels, int intraOpThreads)
    {
        int outputPlane = checked(outputHeight * outputWidth);
        int blocks = outputChannels / 8;
