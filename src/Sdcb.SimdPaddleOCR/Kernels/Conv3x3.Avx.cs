...
{
    // Process two adjacent eight-channel tiles together.  The detector's
    // final 64->16 3x3 projection is large enough that reloading the same
    // input patch for the second tile is measurable; keeping sixteen output
                ...
                for (int co = 0; co < outputChannels; co += 16)
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        ...
                        while (x < width)
                        {
                            ...
                            if (vector)
                            {
                                Vector256<float> a0 = Vector256.Create(b0), a1 = Vector256.Create(b1);
                                Vector256<float> a2 = Vector256.Create(b2), a3 = Vector256.Create(b3);
                                Vector256<float> a4 = Vector256.Create(b4), a5 = Vector256.Create(b5);
                                Vector256<float> a6 = Vector256.Create(b6), a7 = Vector256.Create(b7);
                                Vector256<float> a8 = Vector256.Create(b8), a9 = Vector256.Create(b9);
                                Vector256<float> a10 = Vector256.Create(b10), a11 = Vector256.Create(b11);
                                Vector256<float> a12 = Vector256.Create(b12), a13 = Vector256.Create(b13);
                                Vector256<float> a14 = Vector256.Create(b14), a15 = Vector256.Create(b15);
                                for (int ci = 0; ci < inputChannels; ci++)
                                {
                                    ...
                                    for (int ky = 0; ky < 3; ky++)
                                    {
                                        ...
                                        {
                                            Vector256<float> value = Avx.LoadVector256(srcRow + ix + kx);
                                            int wi = wb + ky * 3 + kx;
                                            ...
                                            a8 = AddMul(a8, value, w8[wi]); a9 = AddMul(a9, value, w9[wi]);
                                            a10 = AddMul(a10, value, w10[wi]); a11 = AddMul(a11, value, w11[wi]);
                                            a12 = AddMul(a12, value, w12[wi]); a13 = AddMul(a13, value, w13[wi]);
                                            a14 = AddMul(a14, value, w14[wi]); a15 = AddMul(a15, value, w15[wi]);
                                        }
                                    }
                                }
            ...
            for (int b = 0; b < batch; b++)
                for (int co = 0; co < outputChannels; co += 4)
                {
                    ...
                    for (int y = 0; y < height; y++)
                    {
                        ...
                        while (x < width)
                        {
                            bool vector16 = y > 0 && y + 1 < height && x > 0 && x + 16 < width;
                            if (vector16)
                            {
                                Vector256<float> a0l = Vector256.Create(b0), a0h = a0l;
                                Vector256<float> a1l = Vector256.Create(b1), a1h = a1l;
                                Vector256<float> a2l = Vector256.Create(b2), a2h = a2l;
                                Vector256<float> a3l = Vector256.Create(b3), a3h = a3l;
                                for (int ci = 0; ci < inputChannels; ci++)
                                {
                                    float* src = batchInput + ci * plane;
                                    int wb = ci * 9;
                                    for (int ky = 0; ky < 3; ky++)
                                    {
                                        ...
                                        for (int kx = 0; kx < 3; kx++)
                                        {
                                            Vector256<float> valueLow = Avx.LoadVector256(srcRow + ix + kx);
                                            Vector256<float> valueHigh = Avx.LoadVector256(srcRow + ix + kx + 8);
                                            int wi = wb + ky * 3 + kx;
                                            a0l = AddMul(a0l, valueLow, w0[wi]); a0h = AddMul(a0h, valueHigh, w0[wi]);
                                            a1l = AddMul(a1l, valueLow, w1[wi]); a1h = AddMul(a1h, valueHigh, w1[wi]);
                                            a2l = AddMul(a2l, valueLow, w2[wi]); a2h = AddMul(a2h, valueHigh, w2[wi]);
                                            a3l = AddMul(a3l, valueLow, w3[wi]); a3h = AddMul(a3h, valueHigh, w3[wi]);
                                        }
                                    }
                                }
                                Avx.Store(o0 + row + x, a0l); Avx.Store(o0 + row + x + 8, a0h);
                                Avx.Store(o1 + row + x, a1l); Avx.Store(o1 + row + x + 8, a1h);
                                Avx.Store(o2 + row + x, a2l); Avx.Store(o2 + row + x + 8, a2h);
                                Avx.Store(o3 + row + x, a3l); Avx.Store(o3 + row + x + 8, a3h);
                                x += 16;
                                continue;
                            }
                            bool vector = y > 0 && y + 1 < height && x > 0 && x + 8 < width;
    ...
    private static void TryFourOutputs(ReadOnlySpan<float> input, ReadOnlySpan<float> weights,
            ...
            for (int co = 0; co < outputChannels; co += 4)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (x < width)
                    {
                        ...
                        if (vector)
                        {
                            Vector256<float> a0 = Vector256.Create(bias0); Vector256<float> a1 = Vector256.Create(bias1);
                            Vector256<float> a2 = Vector256.Create(bias2); Vector256<float> a3 = Vector256.Create(bias3);
                            for (int ci = 0; ci < inputChannels; ci++)
                            {
                                ...
                                for (int ky = 0; ky < 3; ky++)
                                {
                                    ...
                                    for (int kx = 0; kx < 3; kx++)
                                    {
                                        Vector256<float> value = Load(source, sourceOffset + kx);
                            ...
                            Store(output, outputBatch + co * plane + row + x, a0);
                            Store(output, outputBatch + (co + 1) * plane + row + x, a1);
                            Store(output, outputBatch + (co + 2) * plane + row + x, a2);
                            Store(output, outputBatch + (co + 3) * plane + row + x, a3);
                            x += 8;
                            continue;
                        }
